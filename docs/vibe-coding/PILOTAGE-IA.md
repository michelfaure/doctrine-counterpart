...
Et j'ai soldé le trou d'un mois dans mon journal de calibration. Douze sessions, cinquante-six commits, quatre ADR non versés depuis le 20 juillet — mon propre journal était devenu le Cache sans refresher que ma doctrine passe son temps à traquer ailleurs. Je l'ai récolté depuis les sections méthodo que mon protocole de clôture m'impose d'écrire, donc sur toute la période et pas seulement sur ce dont je me souvenais, et je l'ai rangé par règle plutôt que session par session. Bilan de la période : zéro naissance, deux morts, un amendement. C'est la première fois que mon corpus rétrécit.
