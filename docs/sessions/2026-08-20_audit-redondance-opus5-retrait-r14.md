# 2026-08-20 — Audit de redondance Opus 5 : R14 et R5 retirées, Am.R4, et la prémisse retournée
...
- **Propagation aux surfaces vivantes** (le dépôt public est la moins opérante) : skill `challenger` → **étape 4bis** (énumérer écrivains, lecteurs, diff des gardes, écrire le compte) + **déclencheur élargi au fix de finding de review**, qui était le trou réel ; règle R19 user-scope → **cadence UNE passe par défaut** ; `~/CLAUDE.md` et le CLAUDE.md projet → « 19 règles » corrigé en 17 ; feedback du critère d'arrêt + index mémoire alignés ; 2 pointeurs de règles inversés corrigés dans le skill.
...
5. **Pas de bump de version.** Le cycle n'est pas clos (ledger non resynchronisé) : la cadence est datée « hors cycle de version » plutôt que faussement attribuée à une v0.12.
...
7. **R5 RETIRÉE — recouvrement natif, pas falsification.** Deux membres mesurés séparément : le pushback humain est porté quasi mot pour mot par le system prompt du harnais sur Opus 5 (doubler le harnais se compose avec lui sans rien acheter) ; le membre « une mémoire invoquée est une claim falsifiable » n'est **pas** natif et est **fusionné dans R1**, dont il partageait déjà la forme logique et la moitié du texte. Effets de bord traités : R12 ne pointe plus sur un slot mort, 4 pointeurs dans les skills vivants, 4 dans un skill publié, 2 dans le manifesto, README, CONTRIBUTING, checklist (29 → 28).
8. **R3 GARDÉE — révision d'une conclusion posée plus tôt dans la même session.** Trois faits l'ont sortie de la cible : elle est **qualitative** (dit quoi produire avant, pas combien vérifier après — le guide vise la vérification post-hoc) ; **R17 s'appuie explicitement sur elle** (« R17 governs the assertion surface *once the criterion exists* »), et R17 est la règle la mieux fondée du dernier cycle (N=4) ; son absence de provenance signifie *jamais mesurée*, pas *ne mord pas*. Une condition de mesure lui est écrite au ledger, faute de quoi l'absence se reconduirait indéfiniment.
...
- **Condition de mesure de R3** : compter, sur les prochains lots, ceux qui ont démarré sans critère de succès énoncé et ont dû être repris pour cette raison. Zéro sur un trimestre → R3 devient candidate au retrait par vacuité, le chemin de R14. `[unverified]` : aucune mesure n'existe à ce jour.
...
**Une claim de classement est un Cache sans refresher.** Le manifesto affirmait que R5 était *« la deuxième règle la plus invoquée du projet »* — argument qui aurait dû bloquer son retrait. Mesuré le jour même : R5 est **5ᵉ** dans les traces publiques datées (10 occurrences contre 29 pour R19, 22 pour R1, 20 pour R15, 15 pour R18) et **6ᵉ** dans l'usage privé réel. La claim datait d'un corpus de 8 à 14 règles et n'avait jamais été re-mesurée une fois R15, R17, R18 et R19 créées. Un classement vieillit silencieusement à chaque ajout : il se re-mesure ou il ment. Sans cette sonde, une affirmation périmée du corpus aurait protégé une règle que les données ne soutenaient plus.
