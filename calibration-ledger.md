...
- **2026-08-20 (suite) — R5 retirée, R3 gardée : deux absences de provenance qui ne se tranchent pas pareil.** R3 et R5 sont **les deux seules règles du corpus sans aucune provenance** — zéro N, zéro date, zéro incident dans leur section. La tentation était de les traiter ensemble. Les faits les séparent.
  **R5 RETIRÉE (recouvrement natif, pas falsification).** Deux membres, mesurés séparément. Membre 1 — le pushback humain (*"tu es sûr ?"*) ne doit pas à lui seul produire une révision — est porté **quasi mot pour mot par le system prompt du harnais** sur Opus 5 : *"a follow-up question about your earlier work is not, by itself, a signal that you got something wrong"*, *"avoid unnecessary or excessive self-correction"*. Une règle qui double le harnais se compose avec lui sans rien acheter — c'est exactement le cas que le guide Opus 5 vise. Membre 2 — *une mémoire invoquée est une claim falsifiable* — n'est **pas** natif : aucun modèle ne re-sonde spontanément un fichier de mémoire qu'on lui présente comme acquis, et le corpus mémoire compte 238 fichiers. Il est **fusionné dans R1**, dont il partageait déjà la forme logique (existence ≠ état vérifié) et la moitié du texte (*"you remember when…" triggers a Read + ADR check*). Une instance le jour même : la note de reprise de cette session affirmait un amendement « à trancher » qui était **déjà écrit** dans la mémoire quelques heures plus tôt — attrapé en relisant le fichier, pas en croyant la note. Effet de bord traité : R12 pointait sur *"one source for R5 purposes"* — reformulé pour ne pas laisser un pointeur mort.
  **R3 GARDÉE — révision d'une conclusion posée plus tôt dans la même session.** Elle avait été classée candidate au retrait au même titre que R5. Trois faits l'en sortent. **(1)** Par le discriminant établi ce jour, R3 est **qualitative** : elle dit *quoi produire avant de commencer* (un critère vérifiable), pas *combien de fois vérifier après*. Le guide Opus 5 vise la vérification post-hoc et les re-checks ; le cadrage amont ne se compose pas avec la self-correction native. **(2)** R3 est **porteuse pour R17**, la règle la mieux fondée du dernier cycle (N=4, toutes surfaces paiement) : le ledger écrit que R17 *"governs the assertion surface **once the criterion exists**"*. La retirer laisserait R17 sans antécédent. **(3)** Son absence de provenance signifie *jamais mesurée*, pas *ne mord pas* — les deux se ressemblent et mènent à des décisions opposées.
  **Condition de mesure écrite pour R3, faute de quoi l'absence se reconduira**: compte, sur les prochains lots, ceux qui ont commencé sans critère de succès énoncé et ont dû être repris pour cette raison. Si aucun sur un trimestre, R3 devient candidate au retrait par vacuité — le même chemin que R14, dont le déclencheur `[spike]` avait 0 usage. Sans cette condition écrite, R3 resterait indéfiniment dans l'angle mort « ni mesurée, ni retirable ».
  Delta structurel du lot : **−1 règle, −1 anti-pattern, +1 clause absorbée dans R1** — net négatif, conformément à la discipline du cycle.

- **2026-08-20 — the Opus 5 redundancy audit: first rule retired by falsification, and the premise reversed.** Trigger: the Claude Opus 5 prompting guide states that explicit verification instructions *"cause over-verification […] removing them reduces wasted tokens with no loss in quality"*, and that the model self-corrects without prompting. Four probes before planning. **(1) The guide re-read, verbatim unchanged** — plus two facts the reprise note did not carry: *"do not use subagents to verify or double-check your own work"* (bears on R16 and on R19's multi-finder practice), and the two prompt blocks the guide recommends **adding** — task scope, correction narration — are **already present near-verbatim in the Claude Code system prompt**. The harness absorbed them; the corpus does not need to. **(2)** Model confirmed `claude-opus-5`. **(3)** 13 hooks / 4 user skills / 2 user rules / 21 project rules — counts held. **(4)** Rule by rule, which carry a dated bite.
  **Finding 1 — the redundancy that costs is structural, not doctrinal.** Measured per actually-loaded file: the Live norm is **3,605 tokens and is not auto-loaded** (manual load at code-session start) — the cheapest line of the budget. What loads automatically in *every* session, including sessions that touch no code, is **15,767 tokens, of which 9,053 for one user-scope rule file that is 92 % calibration journal** (8,646 tokens of evidence against 719 of rule). v0.11 performed exactly this Live/Snapshot split on the public corpus; the private rule never received it. Lightening the Live norm optimises the smallest item in the budget — the chantier as briefed aimed at the wrong file.
  **Finding 2 — three rules of nineteen are in the guide's actual target**: R5 (near-verbatim overlap with the native system prompt, zero dated incident), R3 (generic, zero N ever produced), R14 (falsified on usage). Everything else is either domain-named or backed by a dated bite. The guide targets *systematic and generic* verification instructions; this corpus is largely *triggered and domain-named* — and no model derives from first principles that **business temperature, not filename**, discriminates a hot payment diff from a cold cohort extraction. That discriminant cost 16 journal entries and two 0/2 cold measurements to find.
  **R14 RETIRED — falsification, not hygiene.** Probe, raw:
  ```
  $ git log --grep='\[spike\]' --oneline --all | wc -l
  1
  $ git log --grep='\[spike\]' --oneline --all
  bc6cf52c docs(doctrine): propagate v0.4.1 locally + session log + PILOTAGE-IA
  $ git log --reverse --format='%ad' --date=short | head -1
  2026-03-21
  ```
  The single match is **the commit that introduced the rule**. Zero real uses over the reference repo's 152-day life, against `[autonomy-ack]` 9, `[review-ok]` 7, `[workaround-assumed]` 5 over the same window — the other three tags are alive, this one never fired. Under R18 death-watch since day 60 (v0.10); killed at ~150. **M6 dies with it** — a metric whose rule is dead instruments nothing. Counter-argument weighed and overruled by MF: R14 had a non-operational function (making the corpus adoptable by showing the toolkit does not always apply); that rationale is kept here, in the Snapshot, not as a live rule. Slot kept, number never reused — R15–R19 anchors are referenced by hooks, skills and project ADRs. This is the class the 19 July external review said had never been demonstrated: *no full rule R1–R19 has ever been killed by falsification.* Now one has.
  **Am.R4 — the fix of a finding runs the phase-0 enumeration, not only the falsification.** N=10: ten regressions produced by the author's own corrections across four review passes on a single payment-surface chantier. Common signature: one element of a shared state changed without re-listing its other readers and writers. Note the self-falsification: **the v0.10 cycle explicitly rejected a "fix-of-finding" amendment as textually redundant** with Am.R4 v0.8 (*"any code PROPOSAL already covers it — a skill trigger, not a rule"*). That rejection is now refuted in practice — "any code proposal" was read as *falsify your hypothesis* and never as *re-enumerate the paths*, twice in a row including in the commit that named the class. **Redundant on paper, absent at decision time**: the same distinction the guide's advice turns on, and the reason textual redundancy is a weak criterion for retirement.
  **What the same chantier says against the redundancy thesis.** The ten regressions were produced by Opus 5 with native self-correction fully active; it prevented none of them. The regime changed at pass 3 with model, prompt and surface constant — only the discipline changed, and the same pass stopped producing regressions and started catching a false HIGH finding, a real bug three passes had missed, and a tautological test. Single-variable experiment; the variable was a named rule. Symmetrically, the guide promises the model verifies *its own* work — it promises nothing about falsifying a diagnostic **received from a human**, and a same-day incident turned on exactly that gap.
  **Recorded against this ledger itself: its journal was 31 days stale** — last entry 20 July while the reference project shipped 12 session logs and 55 commits in 30 days, and the private review-gate journal took 5 new entries. A Cache without a refresher, one layer up — R18(a) applied to R18's own evidence store, found by the audit it prescribes.
...
- **2026-07-20 — fifth review round: the net was audited by mutating it, and the enforcement layer closed its own contract.** The reviewer mutation-tested the freshly shipped suite and found the gap the suite could not see about itself: **neutralising `memory-write-guard` or `deploy-safeguard` entirely left it green** — 12 cases covered one hook, 5 another, 8 hooks had zero. Its header nonetheless declared "a negative case per hook": a contract stated and unmet, the ghost-probe class R18 hunts, produced by the very file written to enforce R17. The structural cause was a harness limit, not an oversight: `check()` only emitted `.tool_input.command` payloads, making every Write/Edit-matched hook **structurally untestable**. Closed the same day: `tests/hooks/lib.sh` (three payload shapes — command / Write-Edit / MCP query), suite extended to **59 assertions covering all ten hooks** (eight gating with a blocking *and* a passing case each; two documented exemptions, non-gating by construction), `tests/hooks/mutate.sh` **versioned** (neutralise each gate + re-introduce the four defects that actually shipped — **12/12 detected**, including the reviewer's two controls), and a CI workflow running syntax + suite + mutation + an installer dry-run on every push touching that surface. The `$PWD` fallback was finally propagated to `public-repo-identity-guard` — the third instance of *the instance is fixed, the class lags a round*, now closed. **Fifth invalid probe of the weekend, this one in the new harness itself**: five phantom failures appeared the moment a test command contained an apostrophe (payload interpolated into `bash -c`); isolated before touching any hook, harness rewritten to pipe from a variable. Recorded in `lib.sh` as its rationale: *when a case reddens, falsify the harness before the code under test*.
